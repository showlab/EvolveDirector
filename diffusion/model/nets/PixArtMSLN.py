...
from diffusion.model.nets.PixArt_blocks_ln import t2i_modulate, CaptionEmbedder, WindowAttention, MultiHeadCrossAttention, T2IFinalLayer, TimestepEmbedder, SizeEmbedder
...
class PixArtMSBlockLN(nn.Module):
    """
...
class PixArtMSLN(PixArt):
    ...
    def __init__(self, input_size=32, patch_size=2, in_channels=4, hidden_size=1152, depth=28, num_heads=16, mlp_ratio=4.0, class_dropout_prob=0.1, learn_sigma=True, pred_sigma=True, drop_path: float = 0., window_size=0, window_block_indexes=None, use_rel_pos=False, caption_channels=4096, lewei_scale=1., config=None, model_max_length=120, **kwargs):
        ...
        self.blocks = nn.ModuleList([
            PixArtMSBlockLN(hidden_size, num_heads, mlp_ratio=mlp_ratio, drop_path=drop_path[i],
...
#################################################################################
@MODELS.register_module()
def PixArtMSLN_XL_2(**kwargs):
    return PixArtMSLN(depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)
