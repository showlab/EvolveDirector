...
class DatasetMS(InternalData):
    def __init__(self, root, image_list_json=None, transform=None, resolution=1024, load_vae_feat=False, aspect_ratio_type=None, start_index=0, end_index=100000000, ad_model_name=None, **kwargs):
        ...
        for json_file in image_list_json:
            ...
            meta_data = list(filter(lambda item: item.get("ad_model") == ad_model_name, meta_data))
            meta_data_clean = [item for item in meta_data if float(item['ratio']) <= 4]
            self.meta_data_clean.extend(meta_data_clean)
            self.img_samples.extend([os.path.join(self.root.replace(self.json_dir_name, self.img_dir_name), os.path.basename(item['path'])) for item in meta_data_clean])
...
def extract_caption_t5(json_path, t5_model):
    global t5
    global t5_save_dir
...
def extract_img_vae(json_path, dataset_root, vae, ad_model_name):
    train_data_json = json.load(open(json_path, 'r'))
    train_data_json_filted = list(filter(lambda item: item.get("ad_model") == ad_model_name, train_data_json))
    image_names = set()
    ...
    if not len(train_data_json_filted) == 0:
        for item in train_data_json_filted:
    ...
    print(f"finish all of {json_path}")
...
def extract_img_vae_multiscale(json_path, dataset_root, ad_model_name, bs=1):
    ...
    dataset = DatasetMS(dataset_root, image_list_json=[json_path], transform=None, sample_subset=None,
                        aspect_ratio_type=aspect_ratio_type, start_index=args.start_index, end_index=args.end_index, ad_model_name=ad_model_name)
...
def get_args():
    ...
    parser.add_argument('--t5_save_root', default='data/evolve_director/caption_feature_wmask', type=str)
    parser.add_argument('--vae_save_root', default='data/evolve_director/img_vae_features', type=str)
    parser.add_argument('--pretrained_models_dir', default='/path/to/Edgen', type=str)
...
if __name__ == '__main__':
    ...
    t5 = T5Embedder(device="cuda", local_cache=True, cache_dir=f'{args.pretrained_models_dir}', model_max_length=120)
    ...
    while True:
        if os.path.exists(ex_fea_singal_path):
            with open(ex_fea_singal_path, "r") as f:
        ...
        json_path = os.path.join(save_root_folder_withtime, "new_data.json")
        if not os.path.exists(json_path):
            print('new_json_file not exists')
            continue

        print("parsing json files: {}.json".format(json_path))

        extract_caption_t5(json_path, t5)

        for TASK in TASKs:

            dataset_root = os.path.join(singal_folder, f'advanced_images_{TASK}', 'imgs')

            # prepare extracted image vae features for training
            if args.multi_scale:
                print(f'Extracting Multi-scale Image Resolution based on {image_resize}')
                extract_img_vae_multiscale(json_path, dataset_root, TASK, bs=1)    # recommend bs = 1 for AspectRatioBatchSampler
            else:
                print(f'Extracting Single Image Resolution {image_resize}')
                extract_img_vae(json_path, dataset_root, vae, TASK)
