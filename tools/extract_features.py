...
def extract_img_vae():
    ...
    for image_name in tqdm(lines):
        ...
        if os.path.exists(f"{save_path}.npy"):
            continue
        try:
            img = Image.open(f'{args.dataset_root}/{os.path.basename(image_names)}')
...
def extract_img_vae_multiscale(bs=1):
    ...
    dataset = DatasetMS(args.dataset_root, image_list_json=[args.json_path], transform=None, sample_subset=None,
