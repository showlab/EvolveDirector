...
def set_env(seed=0):
    ...
    for _ in range(30):
        torch.randn(1, latent_dim, args.image_size, args.image_size)
    ...
    for chunk in tqdm(list(get_chunks(items, bs)), unit='batch'):
        ...
        with torch.no_grad():
            ...
            if args.sampling_algo == 'iddpm':
                ...
                n = len(prompts)
                z = torch.randn(n, latent_dim, latent_size_h, latent_size_w, device=device).repeat(2, 1, 1, 1)
                ...
                # Create sampling noise:
                n = len(prompts)
                z = torch.randn(n, latent_dim, latent_size_h, latent_size_w, device=device)
                ...
                samples = sa_solver.sample(
                    S=25,
                    batch_size=n,
                    shape=(latent_dim, latent_size_h, latent_size_w),
                    eta=1,
...
if __name__ == '__main__':
    ...
    if args.image_size == 512 or args.image_size == 256:
        model = PixArtLN_XL_2(input_size=latent_size, lewei_scale=lewei_scale[args.image_size]).to(device)
    else:
        model = PixArtMSLN_XL_2(input_size=latent_size, lewei_scale=lewei_scale[args.image_size]).to(device)
